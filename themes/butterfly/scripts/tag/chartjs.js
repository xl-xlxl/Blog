...
const chartjs = (args, content) => {
  ...
  const [width = "", layout = false, chartId = ""] = selfConfig
    .split(",")
    .map((s) => s.trim());
  ...
  const renderedDesc = descContent
    ? hexo.render.renderSync({ text: descContent, engine: "markdown" }).trim()
    : "";

  const descDOM = renderedDesc
    ? `<div class="chartjs-desc">${renderedDesc}</div>`
    : "";
